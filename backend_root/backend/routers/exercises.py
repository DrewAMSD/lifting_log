...
def muscle_in_db(cursor: Cursor, muscle: str) -> bool:
    cursor.execute("SELECT * FROM muscles WHERE name = ?", (muscle,))
    return cursor.fetchone() is not None


def insert_exercise_muscles_row(cursor: Cursor, muscle: str, exercise_id: int, is_primary_muscle: bool) -> None:
    cursor.execute("SELECT * FROM muscles WHERE name = ?", (muscle,))
    row = cursor.fetchone()
...
def insert_into_exercise_muscles(exercise: Exercise, cursor: Cursor, exercise_id: int) -> None:
...
def get_exercise_muscles(cursor: Cursor, exercise_id: int, is_primary: bool) -> list[str]:
...
def convert_exercises_row_to_exercise(cursor: Cursor, exercises_row: sqlite3.Row) -> Exercise:
...
def get_exercises(cursor: Cursor, username: str = None) -> list[Exercise]:
    if username is None:
...
def get_exercise(cursor: Cursor, exercise_id: int, username: str = None, for_workout: bool = False) -> Exercise:
    if not username:
...
def get_default_exercises(conn: Annotated[Connection, Depends(get_db)]) -> list[Exercise]:
    cursor: Cursor = conn.cursor()
    return get_exercises(cursor)


@router.get("/exercises/defaults/{exercise_id}")
def get_default_exercise(exercise_id: int, conn: Annotated[Connection, Depends(get_db)]) -> Exercise:
    cursor: Cursor = conn.cursor()
